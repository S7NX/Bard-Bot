...
manager.on("clusterCreate", cluster => {
  console.log(`${chalk.magenta('[SHARDING-MANAGER]:')} Launched Cluster #${cluster.id} | ${cluster.id + 1}/${cluster.manager.totalClusters} [${cluster.manager.shardsPerClusters}/${cluster.manager.totalShards} Shards]`.green);
  ...
  cluster.on("close", function (code) {
    console.log(chalk.red.bold(`Cluster ${cluster.id} closed with code ${code}`));
  });

  cluster.on("exit", function (code) {
    console.log(chalk.red.bold(`Cluster ${cluster.id} exited with code ${code}`));
  });
});
